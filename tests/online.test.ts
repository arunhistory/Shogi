...
describe('authoritative online state',()=>{
  it('applies one legal request exactly once',()=>{
    ...
    const request={requestId:'request_00000001',roomId:'room-a',expectedVersion:0,move};
  ...
  it('rejects stale state and wrong-seat operations without changing state',()=>{
    ...
    const wrongSeat=applyAuthoritativeMove(started,{requestId:'request_00000002',roomId:'room-b',expectedVersion:0,move},'gote');
    expect(wrongSeat).toMatchObject({ok:false,code:'NOT_YOUR_TURN'});
    expect(JSON.stringify(started)).toBe(before);

    const stale=applyAuthoritativeMove(started,{requestId:'request_00000003',roomId:'room-b',expectedVersion:99,move},'sente');
