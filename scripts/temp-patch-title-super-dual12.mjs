...
int32_t super_search_dual12(const Position&root,int maxDepth,int nodeLimit,const int32_t*restrictMoves,int restrictCount){
  ...
  int scores[kMaxMoves];std::atomic<int> nextRoot{0};std::atomic<int> phase{0};std::atomic<int> helperDone{0};std::atomic<bool> stop{false};
  auto runRoots=[&](int depthValue){
    for(;;){
      ...
      const int score=-super_negamax(work,depthValue-1,-kInfinity,kInfinity,perspective,12,1);
  ...
  std::thread helper([&](){
    ...
    for(;;){
      ...
      seen=p;runRoots(seen);helperDone.store(seen,std::memory_order_release);
    }
  });
  ...
  stop.store(true,std::memory_order_release);phase.fetch_add(1,std::memory_order_release);helper.join();
  g_super_best_move=bestMove;g_super_best_score=bestScore;return bestMove;
}
