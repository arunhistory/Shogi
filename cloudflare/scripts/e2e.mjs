...
async function main(){
  ...
  assert.equal(created.data.senteHandicap,'even');
  assert.equal(created.data.goteHandicap,'even');
  ...
  assert.equal(joined.data.senteHandicap,'even');
  assert.equal(joined.data.goteHandicap,'even');
  ...
  const dualCreateBody={requestId:'create-e2e-0090',senteHandicap:'two',goteHandicap:'bishop',order:'gote'};
  const dualCreated=await api('/v1/rooms',{method:'POST',body:dualCreateBody});
  assert.equal(dualCreated.response.status,200);
  assert.equal(dualCreated.data.seat,'gote');
  assert.equal(dualCreated.data.senteHandicap,'two');
  assert.equal(dualCreated.data.goteHandicap,'bishop');
  const dualRetry=await api('/v1/rooms',{method:'POST',body:dualCreateBody});
  assert.deepEqual(dualRetry.data,dualCreated.data,'dual handicap retry must be idempotent');
  const dualConflict=await api('/v1/rooms',{method:'POST',body:{...dualCreateBody,goteHandicap:'rook'}});
  assert.equal(dualConflict.response.status,409);
  assert.equal(dualConflict.data.code,'REQUEST_ID_CONFLICT');
  const dualJoined=await api('/v1/rooms/join',{method:'POST',body:{requestId:'join-e2e-0090',passcode:dualCreated.data.passcode}});
  assert.equal(dualJoined.response.status,200);
  assert.equal(dualJoined.data.senteHandicap,'two');
  assert.equal(dualJoined.data.goteHandicap,'bishop');
  const dualCreatorSocket=await openSocket(dualCreated.data.roomId,dualCreated.data.playerToken);
  assert.equal((await dualCreatorSocket.inbox.until(v=>v.type==='authenticated')).seat,'gote');
  const dualState=await dualCreatorSocket.inbox.until(v=>v.type==='state'&&v.state.status==='playing');
  assert.equal(dualState.state.senteHandicap,'two');
  assert.equal(dualState.state.goteHandicap,'bishop');
  assert.equal(hasPiece(dualState.state,'sente','rook'),false,'sente rook must be removed by sente handicap');
  assert.equal(hasPiece(dualState.state,'sente','bishop'),false,'sente bishop must be removed by sente handicap');
  assert.equal(hasPiece(dualState.state,'gote','rook'),true,'gote rook must remain for bishop handicap');
  assert.equal(hasPiece(dualState.state,'gote','bishop'),false,'gote bishop must be removed independently');
  dualCreatorSocket.socket.terminate();
  ...
  assert.equal(customCreated.data.senteHandicap,'two');
  assert.equal(customCreated.data.goteHandicap,'even');
  ...
  assert.equal(customJoined.data.senteHandicap,'two');
  assert.equal(customJoined.data.goteHandicap,'even');
  ...
  assert.equal(customGoteState.state.senteHandicap,'two');
  assert.equal(customGoteState.state.goteHandicap,'even');
  assert.equal(customGoteState.state.order,'gote');
  assert.ok(Number.isSafeInteger(customGoteState.state.startedAt));
  assert.equal(hasPiece(customGoteState.state,'sente','rook'),false,'legacy sente rook must be removed');
  assert.equal(hasPiece(customGoteState.state,'sente','bishop'),false,'legacy sente bishop must be removed');
  assert.equal(hasPiece(customGoteState.state,'gote','rook'),true,'legacy gote rook must remain');
  assert.equal(hasPiece(customGoteState.state,'gote','bishop'),true,'legacy gote bishop must remain');
